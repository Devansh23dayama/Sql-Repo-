...
-- for practice not related to table 
select emp.enmae from employee as emp join department as dept where emp.dept=dept.dept and emp.salary >dept.salary; 




-- Correlated SubQuery :- iiner query takes value from outer query to change result for everytime.alter
...
select avg(salary),department_name from employee group by department_name;
...
-- get emp name , salary and dep name ,jiska khudka salary uske department ki minimum salary ke equal ho ;
